...
import { ArrowLeft, Settings, Plus, BarChart2, Shield } from "lucide-react";
...
function ChatSidebarMiddle() {
  ...
  const todaySessions = dated
    .filter(({ t }) => t >= today.getTime())
    .map(({ s }) => s);
  const yesterdaySessions = dated
    .filter(({ t }) => t >= yesterday.getTime() && t < today.getTime())
    .map(({ s }) => s);
  const olderSessions = dated
    .filter(({ t }) => t < yesterday.getTime())
    .map(({ s }) => s);
